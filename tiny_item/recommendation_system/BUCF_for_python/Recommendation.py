...
class Recommendation :
    ...
    def max_recommend(self, user_neigh, UL, IL) :
    #   recommand 10 max rated items
    #   input : UL - user-item list
    #           IL - item-user list
    #           uesr_neigh - each item for k neighbours
    #   output : recData - dataSet that recommendation provides
        numRec = 10
        recData = []
        for user in UL.keys() :
            ...
            for j in IL.keys() :
                if j not in UL[user] :
                    interest[j] = 0
            for i in range(len(user_neigh[user])) :
                for item in UL[user_neigh[user][i][0]] :
                    if item not in UL[user] :
                        interest[item] += user_neigh[user][i][1]
            rec = []
