...
class Ploting() :
    ...
    #   plot recall and precision in dataSet
        ...
        p1 = ax.plot(self.K, self.prec[0:6], 'bo--')
        bx = fig.add_subplot(111)
        p2 = bx.plot(self.K, self.prec[6:13], 'rs--')
        plt.title('basic-user-CF cmp user-CF-IIF on precision&recall', fontsize=16)
        plt.xlabel('number of nearest neighbour')
        plt.ylabel('performance')
        plt.legend((p1[0], p2[0]), ('CF', 'CF-IIF'), 'best', numpoints=1)
        plt.show()
    # --- end of plot_data ---
    ...
    #   plot coverage in dataSet
        ...
        plt.title('basic-user-CF cmp user-CF-IIF on coverage', fontsize=16)
        plt.xlabel('number of nearest neighbour')
        plt.ylabel('performance')
        plt.legend((p1[0], p2[0]), ('CF', 'CF-IIF'), 'best', numpoints=1)
        plt.show()
    ...
    #   plot popularity in dataSet
        fig = plt.figure()
        ax = fig.add_subplot(111)
        p1 = ax.plot(self.K, self.pop[0:6], 'co--')
        bx = fig.add_subplot(111)
        p2 = bx.plot(self.K, self.pop[6:13], 'ys--')
        plt.title('basic-user-CF cmp user-CF-IIF on popularity', fontsize=16)
        plt.xlabel('number of nearest neighbour')
        plt.ylabel('performance')
        plt.legend((p1[0], p2[0]), ('CF', 'CF-IIF'), 'best', numpoints=1)
        plt.show()
